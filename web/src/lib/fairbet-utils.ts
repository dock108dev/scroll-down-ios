 ...
 * confidence labelling, and market-key-to-label mapping.
 */
 ...
 * iOS maps: "full" → Sharp, "decent" → Market, "thin" → Thin
 */
 ...
 *   "Over" → "Over"
 *   "Boston Celtics" → "Boston Celtics" (already clean)
 */
function humaniseSelectionKey(key: string, bet: APIBet): string {
  // If key starts with "team:" or "player:", strip prefix and humanise slug
  ...
  // If it's a plain slug with underscores, humanise it
 ...
 *   "Louisville Cardinals Alt Spread -7.5"
 *   "Luka Doncic Points Over 28.5"
 *   "Boston Celtics -3.5"
 */
export function selectionDisplay(bet: APIBet): string {
  // If the server provides a clean bet_description, prefer it
  ...
  if (marketLower.startsWith("player_")) {
    ...
    // selection_key might be "Over" or "Under" for player props
 ...
 * This is what iOS displays as "Estimated Fair Price" (NOT reference_price,
 * which is the sharp sportsbook's line before devig).
 */
