...
- `ScrollDownSports/Models/SDADomainMapper.swift` is 663 LOC. It is the boundary between SDA DTOs and app-domain state, so extraction should happen around tested sport-specific mapping helpers rather than by file size alone.
- `ScrollDownSportsTests/EventAndScoreboardSnapshotTests.swift` is 651 LOC. The suite keeps row and scoreboard visual regressions together through shared snapshot fixtures.
- `ScrollDownSports/Models/SDASituationDomainMapper.swift` is 624 LOC. It maps normalized situation DTOs into app-domain state; split only around tested sport-specific helpers.
...
- `ScrollDownSports/Models/DomainModels.swift` is 597 LOC. The file is a domain model contract; splitting should follow stable feature boundaries, not line count.
...
- `ScrollDownSportsTests/GameDetailViewModelTests.swift` is 503 LOC. The suite is barely over the threshold and keeps detail loading/refresh behavior around shared fixtures.
- `ScrollDownSportsTests/DecodingTests.swift` is 796 LOC. The suite keeps date and DTO decoding coverage together around shared fixtures.
