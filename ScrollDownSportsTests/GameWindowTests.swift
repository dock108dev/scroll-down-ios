...
final class GameWindowTests: XCTestCase {
    ...
    func testHomeWindowEndsAtToday() {
        let now = Date(timeIntervalSince1970: 1_800_000_000)
        ...
        XCTAssertEqual(window.start, calendar.date(byAdding: .day, value: -7, to: today))
        XCTAssertEqual(window.end, calendar.date(byAdding: DateComponents(day: 1, second: -1), to: today))
        XCTAssertTrue(window.contains(now))
        XCTAssertFalse(window.contains(calendar.date(byAdding: .day, value: 1, to: today)!))
    }
}
